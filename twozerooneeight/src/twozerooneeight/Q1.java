...
public class Q1 {
    ...
    public static void towerOfHanoi(int numOfDisks, MyStack <Integer> start, MyStack <Integer> destination, MyStack <Integer> middle){
        if (numOfDisks == 1){
            int disks = start.pop();
            destination.push(disks);
            System.out.println("Move disk " + disks + " from " + start + " to " + destination);
            return;
        }
        
        towerOfHanoi(numOfDisks - 1, start, middle, destination);
        
        int disks = start.pop();
        destination.push(disks);
        System.out.println("Move disk " + disks + " from " + start + " to " + destination);
        
        towerOfHanoi(numOfDisks - 1, middle, destination, start);
    }
}
